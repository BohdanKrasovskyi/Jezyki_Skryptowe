...
class FinanceService:
    ...
    def import_from_plaid(self, account_id: int) -> int:
        from .banking_api import get_plaid_client
        try:
            account = self.acc_repo.get_by_id(account_id)
        except Account.DoesNotExist:
            raise ValueError('Wybrane konto nie istnieje.')
        ...
        raw_txs = client.get_transactions(access_token)
        ext_ids = [t.get('transaction_id', '') for t in raw_txs if t.get('transaction_id')]
        existing = set(
            Transaction.objects.filter(account=account, external_id__in=ext_ids)
            .values_list('external_id', flat=True)
        ) if ext_ids else set()
        ...
        balance_delta = Decimal('0.00')
        for raw in raw_txs:
            ext_id = str(raw.get('transaction_id', ''))
            if ext_id and ext_id in existing:
                continue
            try:
                ...
                continue
            tx_type = Transaction.EXPENSE if raw_amount > 0 else Transaction.INCOME
            amount = abs(raw_amount)
            description = str(raw.get('merchant_name') or raw.get('name') or '')[:255]
            try:
