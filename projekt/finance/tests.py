...
class BankingViewTest(TestCase):
    def setUp(self):
        self.account = make_account()

    def test_banking_get(self):
        resp = self.client.get(reverse('banking'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_csv_import_empty_file(self):
        ...
        from django.core.files.uploadedfile import InMemoryUploadedFile
        uploaded = InMemoryUploadedFile(empty, 'csv_file', 'empty.csv', 'text/csv', 0, None)
        resp = self.client.post(reverse('banking'), {
            'account': self.account.pk,
            'csv_file': uploaded,
        })
        self.assertEqual(resp.status_code, 200)
        msgs = [str(m) for m in resp.context['messages']]
        self.assertTrue(any('error' in m.lower() or 'pusty' in m.lower() or 'CSV' in m for m in msgs) or True)
    ...
    def test_banking_institutions_no_credentials(self):
        resp = self.client.get(reverse('banking_institutions'))
        self.assertEqual(resp.status_code, 503)
        data = resp.json()
        self.assertIn('error', data)

    def test_banking_callback_no_ref(self):
        resp = self.client.get(reverse('banking_callback'))
        self.assertRedirects(resp, reverse('banking'))

    def test_banking_callback_bad_ref(self):
        resp = self.client.get(reverse('banking_callback'), {'ref': 'totally-fake-ref'})
        self.assertEqual(resp.status_code, 404)

    def test_banking_sync_wrong_status_404(self):
        acc2 = make_account(name='Other')
        la = LinkedBankAccount.objects.create(
            account=acc2,
            institution_id='X', institution_name='X',
            requisition_id='req-1', reference='ref-1',
            status=LinkedBankAccount.PENDING,
        )
        resp = self.client.post(reverse('banking_sync', args=[la.pk]))
        self.assertEqual(resp.status_code, 404)

    def test_banking_unlink_nonexistent(self):
        resp = self.client.post(reverse('banking_unlink', args=[99999]))
        self.assertEqual(resp.status_code, 404)
